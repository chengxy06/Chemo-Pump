...
static const int16_t g_scr_change_voice_volume_widgets[] = {
...
*
************************************************/
static void app_scr_change_voice_volume_on_press_ok_key(KeyID key, int repeat_count)
{
    UIView* view;
...
*
************************************************/
static void app_scr_change_voice_volume_on_press_back_key(KeyID key, int repeat_count)
{
    UIView* view;
	view = ui_view_focused();
    if (view->id() == kUI_IDNumberEdit1){
...
*
************************************************/
static void app_scr_change_voice_volume_on_scr_create(Screen* scr)
{
...
*
************************************************/
static void app_scr_change_voice_volume_on_scr_widget_init(Screen* scr, UIView* scr_view)
{
    scr_layout_load_standard(scr, get_string(kStrInputNewVoiceVolume), get_string(kStrReturn), get_string(kStrConfirm));
...
*
************************************************/
static void app_scr_change_voice_volume_on_scr_activate(Screen* scr, UIView* scr_view)
{
    //bind key
    key_set_handler(kKeyOK, kKeyEventPressed, app_scr_change_voice_volume_on_press_ok_key);
    key_set_handler(kKeyBack, kKeyEventPressed, app_scr_change_voice_volume_on_press_back_key);
}
...
*
************************************************/
static void app_scr_change_voice_volume_callback(Screen* scr, ScreenMsg * msg)
{
    switch(msg->msg_id)
    {
        case kUIMsgScrCreate:
            //init all widgets
            app_scr_change_voice_volume_on_scr_create(scr);
            break;
		case kUIMsgScrWidgetInit:
			app_scr_change_voice_volume_on_scr_widget_init(scr, scr->view);
			break;
        case kUIMsgScrActive:
            app_scr_change_voice_volume_on_scr_activate(scr, scr->view);
            break;
...
*
************************************************/
void app_scr_change_voice_volume_enter(void)
{   
    screen_enter(kScreenScrChangeVoiceVolume, app_scr_change_voice_volume_callback,NULL,
        g_scr_change_voice_volume_widgets, ssz_array_size(g_scr_change_voice_volume_widgets));
}
