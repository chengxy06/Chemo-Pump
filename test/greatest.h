...
void greatest_init();
int greatest_parse_args(int argc, const char **argv);
int greatest_report();
...
#define GREATEST_FAILm(MSG)												\
	...
	if(greatest_info.flags&GREATEST_FLAG_ASSERT){assert(0);}
...
#define GREATEST_SKIPm(MSG)                                             \
    do {                                                                \
        if((greatest_info.test_filter == NULL || greatest_info.test_filter[0] == 0)&& \
		(greatest_info.suite_filter == NULL || greatest_info.suite_filter[0] == 0) ){ \
...
#define GASSERT         GREATEST_ASSERT
